...
#ifndef CURIUM_ZCRUWALLET_H
#define CURIUM_ZCRUWALLET_H
...
public:
    CzCRUWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZCRU(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZCRU(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //CURIUM_ZCRUWALLET_H
