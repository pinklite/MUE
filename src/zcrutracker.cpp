...
CzCRUTracker::CzCRUTracker(std::string strWalletFile)
{
...
CzCRUTracker::~CzCRUTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzCRUTracker::Init()
{
...
bool CzCRUTracker::Archive(CMintMeta& meta)
{
...
bool CzCRUTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzCRUTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzCRUTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzCRUTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzCRUTracker::GetSerialHashes()
{
...
CAmount CzCRUTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzCRUTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzCRUTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzCRUTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzCRUTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzCRUTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzCRUTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzCRUTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzCRUTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzCRUTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzCRUTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCRUTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzCRUTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzCRUTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzCRUTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzCRUTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzCRUTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzcru from DB\n", __func__, listDeterministicDB.size());
    }
