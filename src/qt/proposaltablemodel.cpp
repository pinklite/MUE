...
void ProposalTableModel::refreshProposals() {
    ...
    for (CBudgetProposal* pbudgetProposal : bObj)
    {
        ...
        int amount = 0;
		int amount = ValueFromAmount(pbudgetProposal->GetAmount().toInt() * pbudgetProposal->GetTotalPaymentCount().toInt());
...
QVariant ProposalTableModel::data(const QModelIndex &index, int role) const
{
    ...
    case Qt::DisplayRole:
        ...
        case Amount:
            return BitcoinUnits::format(BitcoinUnits::uPHR, rec->amount);
        }
        break;
    case Qt::EditRole:
        switch(index.column())
