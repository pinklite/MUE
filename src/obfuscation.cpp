    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                adds up to a cost of 0.001 CRU per transaction on average.
            */
            if (r <= 10) {
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeCuriumAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("curium-obfuscation");
