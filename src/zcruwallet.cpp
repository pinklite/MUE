...
CzCRUWallet::CzCRUWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zcru seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCRUSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCRUSeed_deprecated()) {
                    LogPrintf("%s: Updated zCRU seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zcru seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zcru wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzCRUWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCRUCount(nCountLastUsed);
    else if (!walletdb.ReadZCRUCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCRUWallet::Lock()
{
    seedMaster = 0;
}

void CzCRUWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCRUWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCRU(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCRUWallet::LoadMintPoolFromDB()
{
...
void CzCRUWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCRUWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCRUWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zcruTracker->HasPubcoinHash(pMint.first)) {
...
bool CzCRUWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCRU(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zcruTracker which also adds to database
    pwalletMain->zcruTracker->Add(dMint, true);
...
void CzCRUWallet::SeedToZCRU(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzCRUWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCRUWallet::GenerateDeterministicZCRU(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCRUWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCRUWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
